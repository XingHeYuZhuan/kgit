...
## 项目集成（通过 JitPack Maven 引入）

通过 JitPack 服务，可以直接将 GitHub 上的 Git 仓库作为依赖项拉取和引入，无需手动管理源码。

1. 在项目根目录的 `settings.gradle.kts` 中添加 JitPack 仓库源：
```kotlin
repositories {
    google()
    mavenCentral()
...
kotlin {
    sourceSets {
        commonMain.dependencies {
            implementation("com.github.XingHeYuZhuan:kgit:main-SNAPSHOT") // 也可以指定具体 Release 版本号（如 "1.0.0"）
        }
    }
}
